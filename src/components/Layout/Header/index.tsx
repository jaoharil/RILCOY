...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 z-40 w-full pb-5 transition-all duration-300 bg-[#25292C] ${sticky ? ' shadow-lg py-5' : 'shadow-none py-6'}`}>
          ...
          <div className="flex items-center gap-4">
            ...
            {isSignInOpen && (
              <div className="fixed top-0 left-0 w-full h-full bg-black/50 flex items-center justify-center z-50">
                <div ref={signInRef} className="relative mx-auto w-full max-w-md overflow-hidden rounded-lg px-8 pt-14 pb-8 text-center bg-white">
                  <button onClick={() => setIsSignInOpen(false)} className="absolute top-0 right-0 mr-8 mt-8 dark:invert" aria-label="Close Sign In Modal">
            ...
            <Link
              href="#"
              className="hidden lg:block bg-primary/15 hover:bg-[#C2C9CC] text-primary hover:text-black px-16 py-5 rounded-full text-lg font-medium"
              onClick={() => {
            ...
            {isSignUpOpen && (
              <div className="fixed top-0 left-0 w-full h-full bg-black/50 flex items-center justify-center z-50">
                <div ref={signUpRef} className="relative mx-auto w-full max-w-md overflow-hidden rounded-lg bg-[#25292C] backdrop-blur-md px-8 pt-14 pb-8 text-center">
                  <button onClick={() => setIsSignUpOpen(false)} className="absolute top-0 right-0 mr-8 mt-8 dark:invert" aria-label="Close Sign Up Modal">
                    <Icon icon="tabler:currency-xrp" className="text-black hover:text-primary text-24 inline-block me-2" />
                  </button>
                  <SignUp />
                </div>
        ...
        <div ref={mobileMenuRef} className={`lg:hidden fixed top-0 right-0 h-full w-full bg-darkmode shadow-lg transform transition-transform duration-300 max-w-xs ${navbarOpen ? 'translate-x-0' : 'translate-x-full'} z-50`}>
          ...
          <nav className="flex flex-col items-start p-4">
            ...
            <div className="mt-4 flex flex-col space-y-4 w-full">
              <Link
                href="#"
                className="bg-transparent border border-primary text-primary px-4 py-2 rounded-lg hover:bg-[#0cc0df] hover:text-white"
                onClick={() => {
                  setIsSignInOpen(true);
                  setNavbarOpen(false);
                }}
